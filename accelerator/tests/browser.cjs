...
(async()=>{
 ...
 for(const l of lessons){
  ...
  assert.ok(await page.evaluate(()=>document.documentElement.scrollHeight<=innerHeight+1),l.id+' lesson fits the viewport');
  await page.evaluate(()=>document.getElementById('st-next').click());
  assert.ok(await page.evaluate(()=>document.documentElement.scrollHeight<=innerHeight+1),l.id+' still fits after stepping');
  console.log('PASS '+l.id+' playback, stop, mobile, '+steps+' steps, one screen');
 }
